...
def searchbar_redirect(request):
    ...
    if query:
        if model == "inventory":
            getstr = "?f=.0.part__name&f=.0.part__friendly_name&f=.0.serial_number&f=.0.old_serial_number&f=.0.location__name&l=.0.icontains&q=.0.{query}"
        elif model == "calibrations":
            if fnmatch(query.strip(), "????-??-??"):
                query = query.strip()
                getstr = (
                    "?f=.0.calibration_event__calibration_date&l=.0.date&q=.0.{query}"
                )
            else:
                getstr = "?f=.0.calibration_event__inventory__serial_number&f=.0.calibration_event__inventory__part__name&f=.0.coefficient_name__calibration_name&f=.0.calibration_event__user_approver__any__username&f=.0.calibration_event__user_draft__any__username&f=.0.notes&l=.0.icontains&q=.0.{query}"
            ...
            getstr = "?f=.0.part_number&f=.0.name&f=.0.friendly_name&l=.0.icontains&q=.0.{query}"
        elif model == "build":
            getstr = "?f=.0.build_number&f=.0.assembly__name&f=.0.assembly__assembly_type__name&f=.0.assembly__description&f=.0.build_notes&f=.0.location__name&l=.0.icontains&q=.0.{query}"
        elif model == "assembly":
            getstr = "?f=.0.assembly_number&f=.0.name&f=.0.assembly_type__name&f=.0.description&l=.0.icontains&q=.0.{query}"
        elif model == "action":
            getstr = (
                "?f=.0.action_type&f=.0.user&f=.0.detail&f=.0.location__name&f=.0.inventory__serial_number&f=.0.inventory__part__name&l=.0.icontains"
                + "&q=.0.{query}"
            )
        elif model == "user":
            ...
            )  # incase provided model is garbage
...
class GenericSearchTableView(LoginRequiredMixin, ExportStreamMixin, SingleTableView):
        ...
        for card_id in card_IDs:
            ...
            for row_id in row_IDs:
                ...
                for field in self.choice_fields:
                    if field in fields:
                        rows[-1]["secret_subrows"] = [
                            dict(field=field, lookup="exact", query=None)
                        ]  # ensure at least one, albeit bogus, query
                        for k, v in self.choice_fields[field]:
                            if (lookup[0] == "exact" and query[0] == v) or (
                                lookup[0] == "icontains"
                                and query[0].lower() in v.lower()
                            ):
                                rows[-1]["secret_subrows"].append(
                                    dict(field=field, lookup="exact", query=k)
                                )
    ...
    def get_queryset(self):
        def userlist_Qkwarg(field, row):
            ...
            if field.startswith("inventory_calibrationevents__latest__"):
                cals_with_matching_users__qs = CalibrationEvent.objects.filter(
                    **{approver_or_draft + "__in": matching_user_IDs}
                )
                inv_latest_cal__subQ = Subquery(
                    CalibrationEvent.objects.filter(inventory=OuterRef("pk")).values(
                        "pk"
                    )[:1]
                )
                all_inv_latest_cal_IDs = (
                    Inventory.objects.all()
                    .annotate(latest_calib=inv_latest_cal__subQ)
                    .values_list("latest_calib", flat=True)
                )
                latest_cals_with_matching_users = cals_with_matching_users__qs.filter(
                    id__in=all_inv_latest_cal_IDs
                )
                inventory_IDs_for__latest_cals_with_matching_users = (
                    latest_cals_with_matching_users.values_list(
                        "inventory__pk", flat=True
                    )
                )
                ...
                Q_kwarg = {
                    "{}__{}__in".format(
                        calib_or_conf, approver_or_draft
                    ): matching_user_IDs
                }
                return Q_kwarg
        ...
        def make_Qkwarg(field, row):
            ...
            userlist_cases = [
                "inventory_calibrationevents__latest__user_approver__any__username",
            ...
            if field in userlist_cases:
                ...
                # default
                Q_kwarg = {
                    "{field}__{lookup}".format(field=field, lookup=row["lookup"]): row[
                        "query"
                    ]
                }
                if field.endswith("__count"):
                    ...
                    Q_kwarg = {
                        "{field}__{lookup}".format(
                            field=field, lookup=row["lookup"]
                        ): int(float(row["query"]))
                    }
                return Q_kwarg
                ...
                fetch_me = [
                    "__".join(subfields.split("__")[:i])
                    for i in range(1, len(subfields.split("__")))
                ]
                fetch_me += [reverse_accessor]

                inv_actions = Accessor.objects.filter(
                    **{reverse_accessor: OuterRef("pk")}
                )
                if select_one in ["__first__", "__latest__"]:
                    ...
                    inv_actions_subquery = Subquery(
                        inv_actions.reverse().values("pk")[:1]
                    )
                all_model_objs = self.model.objects.all().prefetch_related(accessor)
                all_latest_action_IDs = all_model_objs.annotate(
                    latest_action=inv_actions_subquery
                ).values_list("latest_action", flat=True)
                all_latest_actions = Accessor.objects.filter(
                    pk__in=all_latest_action_IDs
                ).prefetch_related(*fetch_me)
                matching_latest_actions = all_latest_actions.filter(
                    **{"{}__{}".format(subfields, row["lookup"]): row["query"]}
                )
                matched_model_IDs = matching_latest_actions.values_list(
                    reverse_accessor + "__pk", flat=True
                )
            ...
            for row in card["rows"]:
                ...
                for field in row["fields"]:
                    ...
                    if field in self.choice_fields:
                        continue  # handled by secret_subrows
        ...
        if final_Q:
            try:
                return (
                    self.model.objects.filter(final_Q)
                    .distinct()
                    .prefetch_related(*self.query_prefetch)
                )
    ...
    def get_context_data(self, **kwargs):
        ...
        avail_lookups = [
            dict(value="icontains", text="Contains"),
    ...
    def get_avail_fields():
        ...
        avail_fields = [
            dict(value="id", text="Database ID", legal_lookup="EXACT_LOOKUP")
        ]
        return avail_fields
    ...
    def get_table_kwargs(self, field_exceptions=[]):
        ...
        try:
            self.exclude_columns = (
                self.request.GET.get("excluded_columns").strip('"').split(",")
            )
        ...
        for field in self.get_avail_fields():
            if (
                ...
                if field["value"] in queried_fields:
                    ...
                    if "extracol" in self.request.GET.getlist("skipcol"):
                        continue
                ...
                if render_func:
                    ...
                    if isinstance(render_func, dict):
                        render_args = (
                            render_func["args"] if "args" in render_func else []
                        )
                        render_kwargs = (
                            render_func["kwargs"] if "kwargs" in render_func else {}
                        )
                        render_func = (
                            render_func["hofunc"]
                            if "hofunc" in render_func
                            else trunc_render
                        )  # higher order function that itself returns a function
                        if (
                            render_func == trunc_render
                            and field["value"] in queried_fields
                        ):
                            targets = [
                                row["query"]
                                for row in flat_rows
                                if row["field"] == field["value"]
                                and row["nega"] is False
                            ]
                ...
                if value_func:
                    if (
                        value_func == "value"
                    ):  # reset to default/base output-value instead of render'd value
                        ...
                        value_kwargs = (
                            value_func["kwargs"] if "kwargs" in value_func else {}
                        )
...
class InventoryTableView(GenericSearchTableView):
    ...
    def get_avail_fields():
        avail_fields = [
            dict(value="part__name", text="Name", legal_lookup="STR_LOOKUP"),
            dict(
                value="part__friendly_name",
                text="Friendly Name",
            ...
            dict(
                ...
                col_args=dict(
                    ...
                    linkify=dict(
                        viewname="inventory:inventory_detail", args=[tables.A("pk")]
                    ),
                ),
            ),
            dict(
            ...
            dict(
                value="build__assembly__name",
                text="Build Name",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="build__build_number",
                text="Build Number",
                legal_lookup="STR_LOOKUP",
            ),
            ...
            dict(
                value="build__is_deployed",
                text="is-build-deployed?",
                legal_lookup="BOOL_LOOKUP",
            ),
            dict(
                value="inventory_deployments__latest__deployment__current_status",
                text="Deployment Status",
                legal_lookup="STR_LOOKUP",
                col_args=dict(
                    render=lambda value: dict(Deployment.DEPLOYMENT_STATUS).get(
                        value, value
                    )
                ),
            ),
            ...
            dict(
                ...
                col_args=dict(
                    ...
                    attrs={"style": "white-space: nowrap;"},
                    linkify=dict(
                        viewname="parts:parts_detail", args=[tables.A("part__pk")]
                    ),
                ),
            ),
            dict(
                value="part__part_type__name",
                text="Part Type",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
            ...
            dict(
                value="part__refurbishment_cost",
                text="Refurb Cost",
                legal_lookup="NUM_LOOKUP",
            ),
            dict(value=None, text="--User-Defined-Fields--", disabled=True),
            dict(
                value="fieldvalues__field__field_name",
                text="UDF Name",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="fieldvalues__field_value",
                text="UDF Value",
                legal_lookup="STR_LOOKUP",
            ),
            dict(value=None, text="--Actions--", disabled=True),
            dict(
                value="actions__latest__action_type",
                text="Latest Action",
                legal_lookup="STR_LOOKUP",
                col_args=dict(
                    render=lambda value: dict(Action.ACTION_TYPES).get(value, value)
                ),
            ),
            dict(
                value="actions__latest__user",
                text="Latest Action: User",
                legal_lookup="STR_LOOKUP",
                col_args=dict(render=lambda value: value.name or value.username),
            ),
            dict(
                value="actions__latest__created_at",
                text="Latest Action: Time",
                legal_lookup="DATE_LOOKUP",
            ),
            dict(
                value="actions__latest__location__name",
                text="Latest Action: Location",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="actions__latest__detail",
                text="Latest Action: Notes",
                legal_lookup="STR_LOOKUP",
                col_args=dict(
                    render=dict(hofunc=trunc_render, kwargs={"safe": True}),
                    value="value",
                ),
            ),
            dict(
                value="actions__count",
                text="Total Action Count",
                legal_lookup="NUM_LOOKUP",
            ),
            dict(value=None, text="--Calibrations--", disabled=True),
            dict(
                value="inventory_calibrationevents__latest__calibration_date",
                text="Latest Calibration Event: Date",
                legal_lookup="DATE_LOOKUP",
                col_args=dict(
                    format="Y-m-d",
                    value="value",  # must specify value for csv download here because of linkify render
                    linkify=lambda record, value: reverse(
                        viewname="exports:calibration",
                        args=[record.inventory_calibrationevents.latest().pk],
                    )
                    if value
                    else None,
                ),
            ...
            dict(
                value="inventory_calibrationevents__latest__user_approver__any__username",
                text="Latest Calibration Event: Approvers",
                legal_lookup="ITER_LOOKUP",
            ),
            dict(
                value="inventory_calibrationevents__latest__user_draft__any__username",
                text="Latest Calibration Event: Reviewers",
                legal_lookup="ITER_LOOKUP",
            ),
            dict(
                value="inventory_calibrationevents__latest__approved",
                text="Latest Calibration Event: Approved",
                legal_lookup="BOOL_LOOKUP",
            ),
        ]
        return avail_fields
    ...
    def get_table_kwargs(self):
        kwargs = super().get_table_kwargs(
            field_exceptions=self.table_class.Meta.udf_accessors
        )
        ...
        for udf in Field.objects.all().order_by("id"):
            ...
            if udf.id not in self.avail_udf:
                continue
    ...
    def get_context_data(self, **kwargs):
        ...
        trashtog = (
            "checked" if "on" in self.request.GET.getlist("trashtog") else "unchecked"
        )
...
class PartTableView(GenericSearchTableView):
    ...
    def get_avail_fields():
        avail_fields = [
            dict(value="name", text="Name", legal_lookup="STR_LOOKUP"),
            dict(
                value="friendly_name", text="Friendly Name", legal_lookup="STR_LOOKUP"
            ),
            dict(
                value="part_number",
                text="Part Number",
            ...
            dict(
                value="part_type__name",
                text="Part Type",
                legal_lookup="STR_LOOKUP",
                col_args=dict(verbose_name="Type"),
            ),
            dict(
                value="revisions__last__revision_code",
                text="Latest Part Revision",
                legal_lookup="STR_LOOKUP",
            ),
            dict(value="unit_cost", text="Unit Cost", legal_lookup="NUM_LOOKUP"),
            dict(
                value="refurbishment_cost",
                text="Refurb Cost",
                legal_lookup="NUM_LOOKUP",
            ),
            dict(
            ...
            dict(
                value="inventory__count",
                text="Inventory Count",
                legal_lookup="NUM_LOOKUP",
                col_args=dict(
                    linkify=lambda record: reverse(viewname="search:inventory")
                    + "?f=.0.part__part_number&l=.0.exact&q=.0.{}".format(
                        record.part_number
                    )
                ),
            ),
            dict(value=None, text="--User-Defined-Fields--", disabled=True),
            dict(
                value="user_defined_fields__field_name",
                text="UDF Name",
                legal_lookup="STR_LOOKUP",
            ),
        ]
    ...
    def get_queryset(self):
        ...
        self.avail_udf.clear()
        [
            [self.avail_udf.add(udf.id) for udf in q.user_defined_fields.all()]
            for q in qs
        ]
        return qs
    ...
    def get_table_kwargs(self):
        kwargs = super().get_table_kwargs(
            field_exceptions=["user_defined_fields__field_name"]
        )
        ...
        for udf in Field.objects.all().order_by("id"):
            ...
            bound_coltup = (
                safename,
                UDF_Column(
                    udf, accessor="user_defined_fields", footer_count=footer_count
                ),
            )
            kwargs["extra_columns"].append(bound_coltup)
        return kwargs
...
class BuildTableView(GenericSearchTableView):
    ...
    def get_avail_fields():
        avail_fields = [
            dict(value="assembly__name", text="Name", legal_lookup="STR_LOOKUP"),
            dict(value="build_number", text="Build Number", legal_lookup="STR_LOOKUP"),
            dict(
                value="assembly__assembly_type__name",
                text="Type",
                legal_lookup="STR_LOOKUP",
            ),
            dict(value="location__name", text="Location", legal_lookup="STR_LOOKUP"),
            dict(
                value="assembly__description",
                text="Description",
                legal_lookup="STR_LOOKUP",
                col_args=dict(
                    render=dict(hofunc=trunc_render, kwargs={"safe": True}),
            ...
            dict(
                value="deployments__latest__current_status",
                text="Deployment Status",
                legal_lookup="STR_LOOKUP",
                col_args=dict(
                    render=lambda value: dict(Deployment.DEPLOYMENT_STATUS).get(
        ...
        avail_fields = [
            dict(
                ...
                col_args=dict(
                    ...
                    attrs={"style": "white-space: nowrap;"},
                    linkify=dict(
                        viewname="assemblies:assembly_detail", args=[tables.A("pk")]
                    ),
                ),
            ),
            dict(value="assembly_number", text="Number", legal_lookup="STR_LOOKUP"),
            dict(
                value="assembly_type__name",
                text="Type",
                legal_lookup="STR_LOOKUP",
                col_args=dict(verbose_name="Type"),
            ),
...
class ActionTableView(GenericSearchTableView):
    ...
    query_prefetch = [
        ...
        "cruise",
        "build",
        "build__assembly_revision__assembly",
        "deployment",
    ]
    ...
    def get_avail_fields():
        avail_fields = [
            dict(value="object_type", text="Object Type", legal_lookup="STR_LOOKUP"),
            ...
            dict(value="location__name", text="Location", legal_lookup="STR_LOOKUP"),
            dict(
                value="inventory__serial_number",
                text="Inventory: Serial Number",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="inventory__part__name",
                text="Inventory: Name",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="build__assembly_revision__assembly__name",
                text="Build Assembly",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
...
class CalibrationTableView(GenericSearchTableView):
    ...
    query_prefetch = [
        "coefficient_name",
        "calibration_event",
        "calibration_event__inventory",
        "calibration_event__inventory__part",
        "calibration_event__inventory__location",
        "calibration_event__user_approver",
        "calibration_event__user_draft",
    ...
    def get_avail_fields():
        avail_fields = [
            dict(
                value="calibration_event__inventory__serial_number",
                text="Inventory: SN",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="calibration_event__inventory__part__name",
                text="Inventory: Name",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="calibration_event__inventory__location__name",
                text="Inventory: Location",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="calibration_event__inventory__assembly_part__reference_designator__refdes_name",
                text="Reference Designator",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="coefficient_name__calibration_name",
                text="Coefficient Name",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="calibration_event__calibration_date",
                text="Calibration Event: Date",
                legal_lookup="DATE_LOOKUP",
            ),
            dict(
                value="calibration_event__user_approver__any__username",
                text="Calibration Event: Approvers",
                legal_lookup="ITER_LOOKUP",
            ),
            dict(
                value="calibration_event__user_draft__any__username",
                text="Calibration Event: Reviewers",
                legal_lookup="ITER_LOOKUP",
            ),
            dict(
                value="calibration_event__approved",
                text="Calibration Event: Approved Flag",
                legal_lookup="BOOL_LOOKUP",
            ),
    ...
    def get_queryset(self):
        ...
        qs = qs.prefetch_related(
            *[
                pf.replace("calibration_event__", "")
                for pf in self.query_prefetch
                if pf.startswith("calibration_event__")
            ]
        )
        qs = qs.select_related("inventory__part__part_type").exclude(
...
class ConfigConstTableView(GenericSearchTableView):
    ...
    query_prefetch = [
        "config_name",
        "config_event",
        "config_event__inventory",
        "config_event__inventory__part",
        "config_event__inventory__location",
        "config_event__user_approver",
        "config_event__user_draft",
    ...
    def get_avail_fields():
        avail_fields = [
            dict(
                value="config_event__inventory__serial_number",
                text="Inventory: SN",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="config_event__inventory__part__name",
                text="Inventory: Name",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="config_event__inventory__location__name",
                text="Inventory: Location",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="config_event__inventory__assembly_part__reference_designator__refdes_name",
                text="Reference Designator",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="config_name__name",
                text="Config/Const Name",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
                value="config_event__configuration_date",
                text="Config/Const Event: Date",
                legal_lookup="DATE_LOOKUP",
            ),
            dict(
                value="config_event__user_approver__any__username",
                text="Config/Const Event: Approvers",
                legal_lookup="ITER_LOOKUP",
            ),
            dict(
                value="config_event__user_draft__any__username",
                text="Config/Const Event: Reviewers",
                legal_lookup="ITER_LOOKUP",
            ),
            dict(
                value="config_event__approved",
                text="Config/Const Event: Approved Flag",
                legal_lookup="BOOL_LOOKUP",
            ),
    ...
    def get_queryset(self):
        ...
        qs = qs.prefetch_related(
            *[
                pf.replace("config_event__", "")
                for pf in self.query_prefetch
                if pf.startswith("config_event__")
            ]
        )
        qs = qs.select_related("inventory__part__part_type").exclude(
            inventory__part__part_type__ccc_toggle=False
...
class InventoryTestResultTableView(GenericSearchTableView):
    model = InventoryTestResult
    table_class = InventoryTestResultTable
    query_prefetch = ["user", "inventory", "inventory__part", "inventory_test"]

    @staticmethod
    def get_avail_fields():
        avail_fields = [
            dict(
                value="inventory_test__name",
                text="Test Name",
                legal_lookup="STR_LOOKUP",
            ),
            dict(
