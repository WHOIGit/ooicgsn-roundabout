            ...
            if custom_fields:
                for field in custom_fields:
                    if field.field_type == "IntegerField":
                        self.fields["udffield_" + str(field.id)] = forms.IntegerField(
                            label=str(field.field_name),
                            required=False,
                            help_text=str(field.field_description),
                        )
                    elif field.field_type == "DecimalField":
                        self.fields["udffield_" + str(field.id)] = forms.CharField(
                            label=str(field.field_name),
                            required=False,
                            help_text=str(field.field_description),
                            validators=[validate_udffield_decimal],
                        )
                    elif field.field_type == "DateField":
                        self.fields["udffield_" + str(field.id)] = forms.DateTimeField(
                            label=str(field.field_name),
                            required=False,
                            help_text=str(field.field_description),
                        ...
                        self.fields["udffield_" + str(field.id)] = forms.BooleanField(
                            label=str(field.field_name),
                            required=False,
                            help_text=str(field.field_description),
                        )
                        ...
                        self.fields["udffield_" + str(field.id)] = forms.ChoiceField(
                            label=str(field.field_name),
                            required=False,
                            help_text=str(field.field_description),
                    ...
                    try:
                        fieldvalue = self.instance.fieldvalues.filter(
                            field_id=field.id
                        ).latest()
...
class ActionRecoverFromDeploymentForm(forms.ModelForm):
    ...
    cruise = forms.ModelChoiceField(
        queryset=Cruise.objects.all(), label="Cruise Recovered On", required=False
    )
...
class InventoryDeploymentForm(forms.ModelForm):
    ...
    deployment_recovery_date = forms.DateTimeField(
        widget=DateTimePickerInput(
            options={
                # "format": "MM/DD/YYYY, HH:mm", # moment date-time format
                "showClose": True,
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            if (
                self.instance.current_status == Action.STARTDEPLOYMENT
                or self.instance.current_status == Action.DEPLOYMENTBURNIN
            ):
    ...
    def clean_deployment_to_field_date(self):
        ...
        deployment_start_date = self.cleaned_data.get("deployment_start_date", None)
        ...
        if deployment_start_date:
            if deployment_to_field_date < deployment_start_date:
                raise forms.ValidationError(
                    "Deployment Cycle dates are invalid. Check that dates are in correct order"
                )
    ...
    def clean_deployment_recovery_date(self):
        print(self.cleaned_data)
        deployment_to_field_date = self.cleaned_data.get(
            "deployment_to_field_date", None
        )
